	...
	public function toxml( $a=null, $format='array' ) {
		if($this->isDev()){
			...
			foreach ($this->results as $item){
				$str = ' uid:'.$item['uid'];
				$str = $str.' arg:'.$item['arg'];
				$str = $str.' title:'.$item['title'];
				$str = $str.' subtitle:'.$item['subtitle'];
				$str = $str.' icon:'.$item['icon'];
				$str = $str.' valid:'.$item['valid'];
				$str = $str.' autocomplete:'.$item['autocomplete'];
				$arr[] = $str;
			}
			print_r($arr);die();
		}
